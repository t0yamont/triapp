...
- **`packages/core/physio` — the pure engine core**: athlete model, threshold detection,
  ...
  **downgrade-only response rules** (§10.2): a bad signal cuts today's load immediately, a
  ...
  layoff, never a silent load drop; and the **weekly re-planning triggers** (§10.3) — adjust
  the plan, not the athlete (cut the target on under-completion, earn the full ramp on
  sustained over-completion, verify an apparent fitness gain with a test, correct distribution
