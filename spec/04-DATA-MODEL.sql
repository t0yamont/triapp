-- =============================================================================
-- IronFlow — Postgres schema (Supabase)
-- Read 03-ALGORITHM.md before changing anything in the athlete-model or plan
-- sections. Column semantics there are load-bearing.
--
-- Conventions:
--   * All timestamps timestamptz (UTC). Local-day logic lives in application code.
--   * All physiological estimates carry value + confidence + provenance + measured_at.
...
create type provenance       as enum ('lab_test','field_test','dfa_a1_multi','cp_model_fit',
                                      'athlete_reported','dfa_a1_single','passive_inference',
                                      'population_formula');
...
create table athlete_zones (
  ...
  -- 5 zones; each {zone, lower, upper, pct_hrr_lower, pct_hrr_upper, pct_hrmax_lower, pct_hrmax_upper}
...
create table races (
  ...
  event_type        text not null,      -- 'sprint_tri','olympic_tri','70.3','ironman','5k',
                                        -- '10k','half_marathon','marathon','ultra','gran_fondo','other'
...
-- Owner policy pattern. profiles keys on id; everything else on athlete_id.
create policy own_profile on profiles
  for all using (id = auth.uid()) with check (id = auth.uid());

-- Example for an athlete-owned table; replicate for each.
create policy own_rows on activities
  for all using (athlete_id = auth.uid()) with check (athlete_id = auth.uid());

-- Child tables reached via parent ownership.
create policy own_streams on activity_streams
  for all using (exists (
    select 1 from activities a
    where a.id = activity_streams.activity_id and a.athlete_id = auth.uid()
  ));

-- Coach policy: present, inert in v1 (permissions.view is false until coach mode ships).
create policy coach_read on activities
  for select using (exists (
    select 1 from coach_athlete_relationships r
    where r.athlete_id = activities.athlete_id
      and r.coach_id = auth.uid()
      and r.status = 'active'
      and (r.permissions->>'view')::boolean is true
  ));

-- ---------------------------------------------------------------------------
-- SCHEDULED JOBS
-- ---------------------------------------------------------------------------
-- Nightly recompute, per athlete local 03:00 — the job resolves local time itself.
select cron.schedule('nightly-recompute', '0 * * * *',
  $$ select net.http_post(url := current_setting('app.edge_url') || '/recompute') $$);
