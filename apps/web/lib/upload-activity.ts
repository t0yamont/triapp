...
import { linkActivityToPlannedWorkout, type TriflowClient } from '@ironflow/api-client';
...
/**
 * Send one file and attribute it. Plain async, no React — the single-file card and the history
 * import both need exactly this, and a hook cannot be called in a loop.
 */
export async function uploadActivityFile(supabase: TriflowClient, file: File): Promise<UploadOutcome> {
  const format = formatOf(file.name);
  if (!format) return { status: 'error', message: `${file.name} isn't a .fit, .tcx or .gpx file.` };
  ...
  try {
    ...
    // Attribute it to a planned session. A failure here must not lose the activity — it is
    // already stored, and can be linked later.
...
export function useActivityUpload(): UseActivityUpload {
  ...
  const upload = useCallback(
    async (file: File): Promise<void> => {
      ...
      setBusy(true);
      setResult(null);
      setResult(await uploadActivityFile(supabase, file));
      setBusy(false);
    },
    [supabase],
  );
